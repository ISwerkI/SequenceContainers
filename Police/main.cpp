...
std::istream& operator>>(std::istream& is, Offence& obj)
{
	...
	obj = Offence(location, time, offence_id);
	return is;
}
...
void main()
{
	...
	std::map<std::string, std::list<Offence>> base =
	{
		...
		std::pair<std::string, std::list<Offence>>("a304bb", {Offence("ул. Парижской Коммуны", "2015.06.16 18:31", 3), Offence("ул. Парижской Коммуны", "2015.06.16 18:31", 4)})
	};
	...
	if (fin.is_open())
	{
		...
		while (!fin.eof())
		{
			...
			for (char* pch = strtok(all_violations, delimiters); pch; pch = strtok(NULL, delimiters))
			{
				...
				s_stream << pch;
				s_stream >> violation;
				base[licese_plate].push_back(violation);
			}
			cout << delimiter << endl;
		}
	}
